"""Train an InvertedDoublePendulum policy with REINFORCE, logging to wandb.
...
def parse_args() -> tuple[HParams, argparse.Namespace]:
    ...
    parser.add_argument("--wandb-project", default="inverted-double-pendulum-reinforce")
...
def build_policy(hp: HParams) -> GaussianPolicy:
    if hp.policy == "mlp":
...
def run_episode(env: gym.Env, policy: GaussianPolicy) -> tuple[torch.Tensor, list[float]]:
    log_probs = []
    rewards = []
    obs, _ = env.reset()
    done = False
    while not done:
        ...
        obs, reward, terminated, truncated, _ = env.step(clipped.numpy())
...
def main() -> None:
    ...
    for episode in range(1, hp.episodes + 1):
        ...
        wandb.log(
            {
                ...
                "train/std": policy.log_std.exp().item(),
            },
            step=episode,
        )
        ...
        if episode % hp.log_every == 0:
            print(
                f"episode {episode:5d}  reward {episode_reward:8.1f}  "
                f"running_reward {running_reward:8.1f}  loss {loss.item():8.3f}"
            )
